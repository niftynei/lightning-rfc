...
Each party to the transaction is responsible for paying the fees for their input, output,
...
`input_info`.`script` is the script signature field for the input.
...
Both nodes:
  - MUST send this message in succession for the collaboration to be considered concluded
...
The sending node:
  ...
  - MUST NOT send a `witness_stack` whose length exceeds the
    `max_witness_len` communicated in `tx_add_input`
...
from the `opener` and they have sent a `commitment_signature`
message, the `accepter`:
...
#### Rationale
The `opener` is responsible for collecting signatures for the
funding transaction and broadcasting it. This allows multi-party
or simultaneous channel establishments to be coordinated by a
single party, who is then responsible for broadcasting the
